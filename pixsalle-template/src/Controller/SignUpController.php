...
final class SignUpController
{
    ...
    public function signUpAction(Request $request, Response $response): Response
    {
        ...
        if (count($errors) == 0) {
            $user = new User("", $data['email'], md5($data['password']), "", "", 0,new DateTime(), new DateTime());
            $this->userRepository->createUser($user);

            $user = $this->userRepository->getUserByEmail($data['email']);
            $userName = "user".$user->id;

            $this->userRepository->updateUserName($data['email'], $userName);
            
            $user = $this->userRepository->getUserByEmail($data['email']);
            
            $userId = $this->userRepository->getUserId($data['email']);
            $this->userRepository->createMembership($userId, "Cool");
            $this->userRepository->setUserMoney($data['email'], 30);
